 ...
 */
public class JavaEnumWithFieldsValuesHandler extends Handler {

    public JavaEnumWithFieldsValuesHandler(Tune tune, MetaInfo metaInfo, RuleSet ruleSet) {
    ...
    public void process(TypeSpecBuilder typeBuilder, TypeModel typeModel) {
        ...
        for (String key : values.keySet()) {
            if (formattedParams == null) {
                ((TypeSpec.Builder)typeBuilder.getBuilder()).addEnumConstant(key);
            } else {
                ((TypeSpec.Builder)typeBuilder.getBuilder()).addEnumConstant(key, TypeSpec.anonymousClassBuilder(formattedParams, values.get(key)).build());
            }
        }
    }
